...
- `/api/fs/list` response fields `filtered_total`, `page`, `per_page`, `has_more`, and `pages_total`: `>= v3.58.0`
- Rename, batch rename, and regex rename are blocked for password-protected meta paths: `>= v3.58.0`
:::
...
> Since `>= v3.58.0`, if the source path is covered by a meta password, AList returns `403` and refuses the rename even if the caller already knows that password.
...
> - `per_page <= 0` uses the backend default `200`
...
> Since `>= v3.58.0`, batch rename also returns `403` if any matched source path is covered by a meta password.
...
> Since `>= v3.58.0`, regex rename also returns `403` if a matched source path is covered by a meta password.
