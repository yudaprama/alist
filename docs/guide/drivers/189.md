...
- `Strip emoji` for sanitized upload / rename / folder creation: `>= v3.55.0`
:::
...
Available in both `189Cloud` and `189CloudPC`.

- When enabled, AList removes four-byte characters from file and folder names before upload, rename, and folder creation. In practice this mainly means emoji and similar characters.
- Use this when Tianyi rejects names containing emoji or other unsupported characters.
- If the sanitized result becomes empty, AList falls back to `file` or `file.ext`.
