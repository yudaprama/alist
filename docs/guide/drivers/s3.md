...
- `Storage class`, storage-class display, and archive / thaw support: `>= v3.54.0`
:::
...
The name of the hidden placeholder object used to keep empty folders. Default: `.alist`.
...
- `Storage class` applies to new uploads and server-side copy operations. The exact supported values depend on your S3 provider.
- Empty folders are represented by a hidden placeholder object. If `Placeholder` is left empty, AList uses `.alist`.
- When the provider returns it, AList also shows each file's storage class in file metadata and API responses.
- Supported S3 providers can archive objects to colder tiers and request thaw / restore later. These operations are asynchronous on the provider side, so completion depends on the storage vendor and selected tier.
