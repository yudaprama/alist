...
- Personal storage: `>= v3.33.0`
- `Safe password` for Safe Box auto-unlock: `>= v3.53.0`
:::
...
Optional. Only fill this in when the mounted folder or files are inside 123 Safe Box.

- When `Safe password` is set, AList will automatically try to unlock the Safe Box and retry the request when a Safe Box prompt is returned.
- The unlocked state is cached for the current AList process.
- If this field is left empty, Safe Box directories or files may fail to list or open.
