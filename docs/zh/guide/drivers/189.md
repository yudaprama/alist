...
`天翼云盘` 和 `天翼云盘客户端` 两个驱动都支持这个选项。

- 开启后，AList 会在上传、重命名、创建文件夹前，先移除文件名中的四字节字符；实际最常见的就是 emoji 和类似字符。
- 如果天翼云盘因为文件名里有 emoji 或其他不兼容字符而报错，可以开启这个选项。
- 如果净化后的名字变成空字符串，AList 会自动回退为 `file` 或 `file.ext`。
